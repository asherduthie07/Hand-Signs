...
class AnimeSpellEngine:
    ...
    def update_slime_chain(self, fid, p1, p2):
        ...
        for _ in range(iterations):
            ...
            for i in range(1, len(nodes)-1):
                ...
                # gravity is split across sub-iterations so it doesn't
                # get added 4x per frame
                curr.vy += gravity / iterations

                curr.vx *= damping
                curr.vy *= damping

                # clamp velocity so a fast hand snap can't make the
                # chain explode outward
                curr.vx = max(-max_speed, min(max_speed, curr.vx))
                curr.vy = max(-max_speed, min(max_speed, curr.vy))
    ...
    def build_slime_mask(self, shape):

        h, w = shape[:2]

        mask = np.zeros((h, w), dtype=np.uint8)

        for nodes in self.slime_chains.values():
            ...
            for i in range(n - 1):
                ...
                p1 = (int(nodes[i].x), int(nodes[i].y))
                p2 = (int(nodes[i + 1].x), int(nodes[i + 1].y))

                t = i / (n - 1)

                # thin at the fingertip anchors, fat in the middle so
                # it reads as a sagging mass of goo, not a straight rod
                bulge = math.sin(t * math.pi)

                thickness = int(5 + bulge * 20)
    ...
    # =====================================================
    # SLIME / DIMENSIONAL RIFT RENDER
    # =====================================================

    def draw_slime_effect(self, frame):
        ...
        raw_mask = self.build_slime_mask(frame.shape)

        if not np.any(raw_mask):
            return

        # melt the individual strand strokes into one soft organic
        # blob instead of leaving hard connected lines
        soft = cv2.GaussianBlur(raw_mask, (0, 0), sigmaX=10, sigmaY=10)

        _, body = cv2.threshold(soft, 50, 255, cv2.THRESH_BINARY)

        # second smoothing pass so the silhouette itself is round,
        # not faceted along the strand's varying thickness
        body = cv2.GaussianBlur(body, (0, 0), sigmaX=6, sigmaY=6)
        _, body = cv2.threshold(body, 90, 255, cv2.THRESH_BINARY)

        # soft edge alpha for translucency
        alpha_body = cv2.GaussianBlur(
            body, (0, 0), sigmaX=5, sigmaY=5
        ).astype(np.float32) / 255.0

        # wide dim halo -> the "rift" bloom leaking into the scene
        glow = cv2.GaussianBlur(
            raw_mask, (0, 0), sigmaX=30, sigmaY=30
        ).astype(np.float32) / 255.0

        # distance-from-edge fakes refraction/fresnel shading:
        # dark violet core, brighter edge. A slow uniform hue drift
        # (not spatial frequency) gives the shimmer without banding.
        dist = cv2.distanceTransform(body, cv2.DIST_L2, 5)

        max_d = dist.max()
        core = dist / max_d if max_d > 0 else dist
        edge = np.clip(1.0 - core, 0.0, 1.0)

        hue_drift = 0.5 + 0.5 * math.sin(t * 0.04)

        core_color = np.array([130, 20, 70], dtype=np.float32)
        edge_color = np.array(
            [255, 70 + 50 * hue_drift, 255],
            dtype=np.float32
        )

        mix = edge[..., None]
        color = core_color * (1 - mix) + edge_color * mix
        color = np.clip(color, 0, 255).astype(np.float32)

        # translucent body pass
        a = (alpha_body * 0.48)[..., None]

        frame[:] = (
            frame.astype(np.float32) * (1 - a) + color * a
        ).astype(np.uint8)

        # additive outer glow
        glow_color = np.array([200, 60, 220], dtype=np.float32)
        g = (glow * 0.28)[..., None]

        frame[:] = np.clip(
            frame.astype(np.float32) + glow_color * g,
            0,
            255
        ).astype(np.uint8)

        # smooth rim on the (now-rounded) silhouette; a morphological
        # gradient stays clean here where Canny would pick up noise
        rim = cv2.morphologyEx(
            body, cv2.MORPH_GRADIENT, np.ones((5, 5), np.uint8)
        )
        rim = cv2.GaussianBlur(rim, (0, 0), sigmaX=1.2, sigmaY=1.2)

        rim_alpha = (rim.astype(np.float32) / 255.0 * 0.9)[..., None]
        rim_color = np.array([255, 210, 255], dtype=np.float32)

        frame[:] = (
            frame.astype(np.float32) * (1 - rim_alpha) +
            rim_color * rim_alpha
        ).astype(np.uint8)

        # sparse drifting sparkle motes for a little magic dust
        ys, xs = np.where(body > 0)

        if len(xs) > 0:

            rng = np.random.default_rng(t // 3)

            count = min(12, len(xs) // 60 + 2)

            picks = rng.choice(len(xs), size=count, replace=False)

            for idx in picks:

                px, py = int(xs[idx]), int(ys[idx])

                flicker = 0.5 + 0.5 * math.sin(t * 0.25 + idx)
        ...
        while True:
            ...
            if results.multi_hand_landmarks:
                ...
                if len(all_hands) == 2:
                    ...
                    slime_pose_now = self.detect_slime_pose(
                        left_lm,
                        right_lm
                    )

                    if slime_pose_now:
                        # fingers are touching -> slime turns/stays on
                        self.slime_active = True

                    elif self.slime_active:
                        # already active but fingers separated -> only
                        # release once the hands are pulled clearly
                        # apart, otherwise the strand keeps stretching
                        wrist_dist = math.sqrt(
                            (left_lm[0].x - right_lm[0].x) ** 2 +
                            (left_lm[0].y - right_lm[0].y) ** 2
                        )
